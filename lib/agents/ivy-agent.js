...
function systemPrompt(role) {
  ...
  const roleLine = role === 'media_buyer'
    ? `The person you are talking to is a MEDIA BUYER. You ONLY handle expense logging for them. If they ask for analytics, profit, or capital balances, tell them that's owner-only. Never state a pool balance.`
    : `The person you are talking to is the brand OWNER. After logging you may state the resulting pool balance.
If they ask about profit, revenue, COGS, or "was this month good?" — call the get_profit_summary tool
and answer from its numbers in one short message. Never invent figures.`;
...
Every expense also has an EXPENSE CLASS — decide it yourself, never ask unless genuinely ambiguous:
- inventory_purchase: buying sellable stock — fabric, materials, manufacturing, production runs,
  restock ("bought fabric", "paid the factory", "restocked hoodies"). It hits cash but NOT this
  month's profit (it becomes cost of goods as units sell).
- opex: everything else — ads, salaries, shipping, packaging, software, rent, fees.
Only when you truly can't tell (e.g. a bare amount with a vague note like "supplier payment"),
confirm in ONE line: "logging EGP 50,000 — is this stock/manufacturing or a running expense?"
...
// Owner-only: the profit summary contains figures media buyers must never see,
// so the tool is not even offered to the model for them.
const OWNER_TOOLS = [
  {
    type: 'function',
    function: {
      name: 'get_profit_summary',
      description: "The brand's real P&L for a period: net revenue (delivered − returned), COGS, opex, real net profit, cash delta, return rate. Use for 'am I profitable?' style questions.",
      parameters: {
        type: 'object',
        properties: {
          period: { type: 'string', enum: ['this_month', 'last_month', 'last_90'], description: "Defaults to this_month." },
        },
        additionalProperties: false,
      },
    },
...
async function profitSummary(brandId, role, args) {
  ...
  try {
    ...
    const period = ['this_month', 'last_month', 'last_90'].includes(args?.period) ? args.period : 'this_month';
