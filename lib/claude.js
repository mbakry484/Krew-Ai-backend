const { client: openai, defaultModel, visionModel } = require('./ai-provider');
const supabase = require('./supabase');
const { buildOptimizedPrompt, analyzeContext } = require('./prompts/prompt-manager');
const { logUsage } = require('./usage-logger');
const { getActiveVoiceProfile } = require('./voice-processor');
...
async function generateReply(
  ...
  try {
    // Fetch active voice profile for this brand (returns null if none)
    const voiceProfile = brandId ? await getActiveVoiceProfile(brandId) : null;
    ...
    const model = imageUrl ? visionModel : defaultModel;
...
STEP 2 — Collect order details (ask for ALL missing fields in ONE message):
  - Full name
  - Phone number
  - Delivery address

  Important: Extract information intelligently from conversation history. If the customer sends multiple fields at once (e.g. "Ahmed\n01012345678\nCairo"), parse all of them. Never ask for information already provided.
...
STEP 4 — After customer confirms:
  ...
  The JSON must be valid, on a single line, with no extra text before or after.
  Fill in actual values — price must be a number, all others are strings.
