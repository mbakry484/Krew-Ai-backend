...
function analyzeContext(customerMessage, conversationHistory = [], metadata = null) {
  ...
  // (e.g. "Mohamed" matching "m", "small" inside "smallest"). Split on non-letter/number
  // chars so trailing punctuation ("size?") still tokenizes and Arabic words stay intact.
  const msgWords = msg.split(/[^\p{L}\p{N}]+/u).filter(Boolean);
...
function buildOptimizedPrompt({
  ...
  if (!isExchangeRefundFlow && (context.isOrderIntent || (metadata && metadata.current_order))) {
    prompt += '\n\n' + orderTaking.getPrompt(metadata, storefrontUrl);
  }
  ...
  if (context.needsProductCatalog || context.isOrderIntent || hasImage) {
    prompt += '\n\n' + productCatalog.getPrompt(inStockProducts, outOfStockProducts, storefrontUrl);

    // Generic catalog-scanning image rules are a FALLBACK for when vector
    // search found nothing. When it did find matches, the IMAGE SEARCH
    // RESULTS section (appended by the caller) is authoritative — including
    // this generic flow alongside it makes Luna scan the (capped) catalog
    // list instead, miss products, and answer "we don't have this".
    if (hasImage && !hasVectorMatches) {
      prompt += '\n\n' + productCatalog.getImageHandlingPrompt();
    }
  }

  // TEXT turns after a photo: re-ground follow-up questions ("what is its
  // name?", "how much?", "send the link") in the last photo's visual matches.
  // Without this the match results evaporate after the image turn and the
  // model answers from older conversation topics instead. Deliberately NOT
  // gated on needsProductCatalog — a bare "what is its name" doesn't trigger
  // catalog keywords.
  const lastImageMatches = metadata && Array.isArray(metadata.last_image_matches)
    ? metadata.last_image_matches : [];
  if (!hasImage && lastImageMatches.length > 0) {
    const matchLines = lastImageMatches.map(m => {
      const itemNote = m.item ? ` (the ${m.item} in the photo)` : '';
      return m.in_stock
        ? `- ${m.name}${itemNote} — ${m.price} EGP ✅ In Stock${m.url ? ` — link: ${m.url}` : ''}`
        : `- ${m.name}${itemNote} — ❌ OUT OF STOCK (do NOT give a price, do NOT allow ordering)`;
    }).join('\n');
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Earlier in this conversation the customer sent a photo. Visual search identified these store products in it:
${matchLines}

- When the customer says "it", "this one", "these", or asks for the name / price / link WITHOUT naming a product, they mean the products above — answer from THIS list, never from older topics in the conversation.
- These are REAL store products even if they don't appear in the catalog list above.
- Write product names EXACTLY as shown here — never translate, transliterate, or reword them.
- Only share the links shown here (or the brand website); never attach another product's link.
- Paste links as PLAIN raw URLs (https://...) — never markdown like [text](URL); Instagram shows the brackets literally.`;
  }
  ...
  if (sizeGuidesEnabled && sizeGuides && sizeGuides.length > 0) {
    prompt += '\n\n';
    prompt += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n📏 SIZE GUIDES — WHEN & HOW TO SHARE\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
    prompt += `CRITICAL SIZE GUIDE RULES:\n`;
    prompt += `1. A size chart is ONLY relevant to a SPECIFIC product. Treat a message as a size-chart request ONLY when the customer is genuinely asking about the sizing/fit/measurements of a product that has been named or is clearly the product currently under discussion.\n`;
    prompt += `2. The mere appearance of the word "size" (or "small/medium/large", etc.) is NOT a size-chart request. Messages like "I'm a size medium and want to see your products" or a lone "size" with no product context must NOT trigger a size chart.\n`;
    prompt += `3. If a customer asks about sizing but it's unclear WHICH product they mean → do NOT send any chart. Ask which product first, e.g. "Which piece are you asking about? I'll pull up its size chart 📏".\n`;
    prompt += `4. When the customer names a product (or one is clearly under discussion) that has a size guide below → the system will automatically send that product's chart image. Your ONLY job is to reply with one short line like "Here's the size chart! 📏". Do NOT describe measurements or mention URLs.\n`;
    prompt += `5. If the product they ask about has NO size guide below → don't fabricate measurements; tell them you'll check with the team.\n\n`;
