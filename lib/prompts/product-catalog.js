...
- Product names are EXACT identifiers: always write a product's name EXACTLY as listed — same language, same spelling, same capitalization. NEVER translate, transliterate, or reword a product name, even when the rest of your reply is in Arabic or any other language.
...
- Send the raw URL as-is (e.g. ${storefrontUrl}). NEVER format a link as markdown — never [text](URL), never [URL](URL) — Instagram DMs show the brackets literally as broken text. Do not shorten URLs either.
`;
    }
