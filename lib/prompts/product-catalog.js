...
- SPECIFIC product questions ("do you have the [X]?", "how much is the [Y]?", "is [Z] available?") → look it up in the catalog below. If it's IN STOCK, share its details (name, price, key info) AND its product link so they can order it on the website. If it's OUT OF STOCK, say so honestly, share its product link so they can check it later, and point them to the website to browse similar pieces.
...
- When you discuss a SPECIFIC product that has a "link:" listed below, ALWAYS share that exact link — in stock or out of stock. For in-stock products encourage them to complete the order on the website; for out-of-stock products the link lets them check the piece and find similar ones.
...
`;
    ...
    // Build out-of-stock product list (links included so the customer can
    // still check the product on the website even when it can't be ordered)
    const outOfStockList = (outOfStockProducts || []).slice(0, 30).map((p, i) => {
      let line = `${i + 1}. ${p.name} - ${p.price} EGP ❌ Out of Stock`;
      const productUrl = buildProductUrl(p, storefrontUrl);
      if (productUrl) {
        line += `\n     ↳ link: ${productUrl}`;
      }
...
- If customer asks about an OOS product → acknowledge it exists, clearly state it's currently unavailable, share its "link:" if listed, then suggest the closest IN STOCK alternatives.
