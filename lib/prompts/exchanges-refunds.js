...
SUMMARY FORMAT (use this exact structure so the team sees it clearly):
  ...
  📝 Reason: [reason]
  🔄 Replacement: [replacement product + variant]
...
⛔ NEVER use the word "refund" when the customer asked for an exchange.`.trim(),
};
...
⛔ NEVER use ESCALATE_EXCHANGE — this is a refund.`.trim(),
...
⛔ NEVER use the word "exchange" when the customer asked for a refund.`.trim(),
};
  ...
  if (flow === 'exchange') {
    switch (step) {
      ...
      case 'pick_replacement':
        ...
        break;
      case 'escalate':
        parts.push(exchangeSteps.escalate());
        break;
      default:
    ...
    switch (step) {
      ...
      case 'suggest_exchange':
        ...
        if (metadata && metadata.exchange_suggested) {
          parts.push(refundSteps.confirmRefund());
        } else {
      ...
      case 'confirm_refund':
        ...
        break;
      case 'escalate':
        parts.push(refundSteps.escalate());
        break;
      default:
        return getPrompt(inStockProducts, outOfStockProducts);
    }
