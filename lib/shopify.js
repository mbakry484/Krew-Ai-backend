...
async function getValidAccessToken(integration) {
  ...
  // Legacy non-expiring token: no expiry and no refresh token.
  // Shopify now hard-rejects these with 403 — store must re-authorize via OAuth.
  if (!token_expires_at && !refresh_token) {
    console.error(`❌ Legacy non-expiring Shopify token detected for ${shopify_shop_domain}. Store must reconnect.`);
  ...
  // Has expiry but no refresh token — expired and unrecoverable
  console.error(`❌ Shopify token expired and no refresh token for ${shopify_shop_domain}. Store must reconnect.`);
  throw Object.assign(new Error('Shopify store must reconnect — token expired with no refresh token.'), { code: 'SHOPIFY_NEEDS_RECONNECT' });
}
