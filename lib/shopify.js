...
async function refreshShopifyToken(shopDomain, refreshToken) {
  ...
  if (updateError) {
    console.error(`❌ Failed to save refreshed Shopify tokens for ${shopDomain}:`, updateError);
    throw new Error(`Failed to persist refreshed Shopify token: ${updateError.message}`);
  }
...
// In-memory cache of tokens fetched from the embedded app: shopDomain → { token, fetchedAt }
const embeddedTokenCache = new Map();
const EMBEDDED_TOKEN_TTL_MS = 30 * 60 * 1000; // re-fetch every 30 min

/**
 * Fetch a fresh, valid expiring offline access token from the embedded Shopify app.
 * The embedded app owns the token lifecycle (it obtains expiring offline tokens via
 * token exchange and auto-refreshes them through the Shopify app library), so it is the
 * single source of truth. We persist the fetched token back to Supabase as a cache.
 */
async function getOfflineTokenFromApp(shopDomain) {
  const appUrl = process.env.SHOPIFY_EMBEDDED_APP_URL;
  const secret = process.env.SHOPIFY_INTERNAL_SECRET;
  if (!appUrl || !secret) return null;

  // Serve from in-memory cache when fresh, and share concurrent fetches via the mutex
  const cached = embeddedTokenCache.get(shopDomain);
  if (cached && Date.now() - cached.fetchedAt < EMBEDDED_TOKEN_TTL_MS) {
    return cached.token;
  }
  if (refreshInFlight.has(shopDomain)) {
    return refreshInFlight.get(shopDomain);
  }

  const fetchPromise = (async () => {
    const res = await fetch(
      `${appUrl.replace(/\/$/, '')}/internal/offline-token?shop=${encodeURIComponent(shopDomain)}`,
      { headers: { 'X-Krew-Secret': secret } }
    );
    if (!res.ok) {
      const body = await res.text();
      console.error(`❌ Embedded app token fetch failed (${res.status}) for ${shopDomain}:`, body);
      throw Object.assign(new Error('Could not obtain Shopify token from embedded app.'), { code: 'SHOPIFY_NEEDS_RECONNECT' });
    }
    const data = await res.json();
    if (!data.access_token) {
      throw Object.assign(new Error('Embedded app returned no access token — store must reconnect.'), { code: 'SHOPIFY_NEEDS_RECONNECT' });
    }

    embeddedTokenCache.set(shopDomain, { token: data.access_token, fetchedAt: Date.now() });

    // Persist back to Supabase as a cache (best-effort)
    ...
    return data.access_token;
  })().finally(() => refreshInFlight.delete(shopDomain));

  refreshInFlight.set(shopDomain, fetchPromise);
  return fetchPromise;
}

/**
 * Get a valid Shopify access token for a shop.
 * Pass the integration row from Supabase.
 *
 * Primary source: the embedded Shopify app (when SHOPIFY_EMBEDDED_APP_URL is configured),
 * which owns expiring-offline-token lifecycle. Falls back to the stored token / refresh-token
 * flow for legacy setups.
 */
async function getValidAccessToken(integration) {
  ...
  // Preferred path: pull a guaranteed-valid token from the embedded app.
  if (process.env.SHOPIFY_EMBEDDED_APP_URL) {
    return getOfflineTokenFromApp(shopify_shop_domain);
  }

  // ---- Legacy fallback (no embedded app configured) ----
  ...
  // No refresh token — could be a legacy non-expiring token or an expired token with no way to refresh.
  // Return it and let the API call surface the 403, which shopifyGraphQL converts to SHOPIFY_NEEDS_RECONNECT.
  console.warn(`⚠️ No refresh token for ${shopify_shop_domain} — returning token as-is, API may reject it.`);
  return access_token;
}
...
async function shopifyGraphQL(shopDomain, accessToken, query, variables = {}) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403 && errorBody.includes('Non-expiring access tokens are no longer accepted')) {
      throw Object.assign(new Error('Shopify store must reconnect — non-expiring tokens are no longer accepted.'), { code: 'SHOPIFY_NEEDS_RECONNECT' });
    }
