...
async function shopifyGraphQL(shopDomain, accessToken, query, variables = {}) {
  ...
  if (!response.ok) {
    ...
    console.error(`❌ Shopify GraphQL ${response.status} | URL: https://${shopDomain}/admin/api/${SHOPIFY_API_VERSION}/graphql.json`);
    console.error(`❌ Response body:`, errorBody);
    console.error(`❌ Token (first 10 chars): ${accessToken?.substring(0, 10)}...`);
