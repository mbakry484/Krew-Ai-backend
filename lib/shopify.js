 ...
 *
 * Tokens are encrypted at rest (lib/crypto.js). The rule in this file:
 * DB values are ciphertext, everything in memory past the read boundary is
 * plaintext. getValidAccessToken() and reloadIntegrationRow() are the two read
 * boundaries — they decrypt, so every caller downstream gets a usable token and
 * never has to know encryption exists. Writes encrypt.
 */

const supabase = require('./supabase');
const { encryptSecret, decryptSecret, decryptRow } = require('./crypto');

const SHOPIFY_API_VERSION = '2026-04';

/**
 * Redact a token for logging. Returns the first 6 characters followed by an
 * ellipsis so a token can be correlated in logs without exposing its value.
 * Intentionally NOT used anywhere by default — reach for it only when you must
 * trace a specific token, and never log the raw value.
 */
...
/**
 * Thrown when Shopify rejects a token refresh call. Carries the HTTP status so
 * callers can distinguish auth-level failures (dead/rotated lineage) from
 * transient network/5xx errors.
 */
class ShopifyRefreshFailedError extends Error {
  constructor(shopDomain, status, body) {
    super(`Shopify token refresh failed for ${shopDomain}: HTTP ${status}`);
    this.name = 'ShopifyRefreshFailedError';
    this.code = 'SHOPIFY_REFRESH_FAILED';
    this.shopDomain = shopDomain;
    this.status = status;
    this.body = body;
  }
...
async function refreshShopifyToken(shopDomain, refreshToken) {
  ...
  if (!response.ok) {
    ...
    throw new ShopifyRefreshFailedError(shopDomain, response.status, body);
  }
  ...
  // Persist new tokens (encrypted at rest; the plaintext is returned to caller)
  await supabase
    .from('integrations')
    .update({
...
/**
 * Re-read the shop's integration row. The embedded Shopify app's SDK shares
 * this row and may have rotated the tokens since our caller read it.
 */
async function reloadIntegrationRow(shopDomain) {
  const { data, error } = await supabase
    .from('integrations')
    .select('shopify_shop_domain, access_token, refresh_token, token_expires_at, refresh_token_expires_at')
    .eq('shopify_shop_domain', shopDomain)
    .eq('platform', 'shopify')
    .maybeSingle();
  if (error) {
    console.error(`Failed to re-read integration row for ${shopDomain}:`, error.message);
    return null;
  }
  // Read boundary — hand back plaintext so comparisons against in-memory
  // tokens (the cross-process lineage checks below) compare like with like.
  return data ? decryptRow(data, ['access_token', 'refresh_token']) : null;
}

function tokenIsFresh(row) {
  return !!row?.token_expires_at &&
    new Date(row.token_expires_at).getTime() - REFRESH_BUFFER_MS > Date.now();
}

/**
 * Refresh with cross-process recovery. Shopify refresh tokens are single-use,
 * and the embedded app's SDK (page loads AND webhook auth) refreshes the same
 * lineage near expiry. On an auth-level refresh failure, re-read the row: if
 * another process already rotated it, adopt the new lineage instead of
 * declaring the store dead.
 */
async function refreshWithRecovery(shopDomain, refreshToken, tokenExpiresAtAtRead) {
  try {
    return await refreshShopifyToken(shopDomain, refreshToken);
  } catch (err) {
    const authFailure =
      err instanceof ShopifyRefreshFailedError &&
      (err.status === 400 || err.status === 401);
    if (!authFailure) throw err; // network/5xx — transient, never mark reconnect-required

    const fresh = await reloadIntegrationRow(shopDomain);
    const rotatedElsewhere =
      fresh &&
      (fresh.refresh_token !== refreshToken ||
        fresh.token_expires_at !== tokenExpiresAtAtRead);

    if (rotatedElsewhere) {
      if (tokenIsFresh(fresh)) {
        console.log(`♻️ ${shopDomain}: token rotated by another process — adopting the new lineage.`);
        return fresh.access_token;
      }
      if (fresh.refresh_token && fresh.refresh_token !== refreshToken) {
        console.log(`♻️ ${shopDomain}: refresh token rotated by another process — retrying once with the new one.`);
        return refreshShopifyToken(shopDomain, fresh.refresh_token);
      }
    }

    // Same lineage we started with and Shopify rejected it — genuinely dead.
    console.error(`🔒 Shopify refresh token rejected for ${shopDomain} — merchant must reconnect the store.`);
    throw new ShopifyReconnectRequiredError(shopDomain);
  }
}
 ...
 * instead of racing with the same single-use refresh token, and recovers
 * when the embedded app's SDK rotated the shared row first.
 */
async function getValidAccessToken(integration) {
  ...
  // THE read boundary for Shopify tokens. Callers all over the codebase read an
  // integrations row themselves and pass it straight in, so decrypting here —
  // rather than at each of those reads — is what keeps encryption from leaking
  // into 40-odd call sites. Safe to re-apply: an already-plaintext value passes
  // through unchanged, which matters because the recursive call below re-enters
  // with an already-decrypted row.
  const access_token = decryptSecret(integration.access_token);
  const refresh_token = decryptSecret(integration.refresh_token);

  // If we have expiry info and the token hasn't expired yet (with 5 min buffer), use it
  if (tokenIsFresh(integration)) {
    return access_token;
  }

  // Token is expired or no expiry info — try to refresh
  if (refresh_token) {
    // If the refresh token itself has expired, no refresh can succeed — but our
    // copy of the row may be stale: the embedded app's SDK may have rotated the
    // lineage since this integration object was read. Check before demanding a
    // reconnect.
    if (refresh_token_expires_at && new Date(refresh_token_expires_at).getTime() <= Date.now()) {
      const fresh = await reloadIntegrationRow(shopify_shop_domain);
      if (fresh?.refresh_token && fresh.refresh_token !== refresh_token) {
    ...
    const refreshPromise = refreshWithRecovery(shopify_shop_domain, refresh_token, token_expires_at)
      .finally(() => {
...
async function getShopifyOrderByNumber(shopDomain, accessToken, orderNumber) {
  ...
  return {
    ...
    line_items: order.lineItems.nodes.map(li => ({
      ...
      unit_price: parseFloat(li.discountedUnitPriceSet?.shopMoney?.amount || '0'),
...
async function migrateLegacyShopifyTokens({ dryRun = false } = {}) {
  ...
  for (const row of rows || []) {
    ...
    try {
      const data = await exchangeOfflineToken(shop, decryptSecret(row.access_token));
...
module.exports = { createShopifyOrder, getShopName, getStorefrontUrl, getShopifyOrderByNumber, shopifyGraphQL, getValidAccessToken, refreshShopifyToken, exchangeOfflineToken, migrateLegacyShopifyTokens, ShopifyReconnectRequiredError, ShopifyRefreshFailedError, SHOPIFY_API_VERSION, redactToken };
