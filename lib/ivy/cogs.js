 ...
 *
 * `allowWithoutShopify` books the order even when Shopify can't resolve it —
 * with zero lines, which makes recordDelivery flag it cogs_incomplete. Bosta
 * ingestion sets this: a delivery whose businessReference doesn't match a
 * Shopify order still collected real cash, and dropping that COD would
 * under-report revenue. Better to count the money and admit the margin is
 * unknown (surfaced via unmatched_bosta_deliveries) than to lose the sale.
 */
async function ensureOrderWithLines(brandId, orderNumber, { trackingNumber = null, allowWithoutShopify = false } = {}) {
  ...
  try {
    ...
    if (integration) {
      ...
      shopifyOrder = await getShopifyOrderByNumber(integration.shopify_shop_domain, accessToken, number);
      if (!shopifyOrder) console.error(`[ivy-cogs] order #${number} not found on Shopify for brand ${brandId}`);
    } else {
      console.error(`[ivy-cogs] no Shopify integration for brand ${brandId} — cannot resolve order #${number}`);
    }
  } catch (err) {
    // A Shopify outage or an expired token must not drop a real delivery on the
    // floor when the caller is willing to book it unmatched.
    console.error(`[ivy-cogs] Shopify lookup failed for order #${number} (brand ${brandId}): ${err.message}`);
    if (!allowWithoutShopify) throw err;
  }

  if (!shopifyOrder && !allowWithoutShopify) return null;

  const { data: order, error: insertError } = await supabase
    .from('ivy_orders')
    .insert({
      brand_id: brandId,
      shopify_order_id: shopifyOrder?.shopify_id ?? null,
      order_number: number,
  ...
  if (insertError) {
    ...
    if (insertError.code === '23505') return ensureOrderWithLines(brandId, number, { trackingNumber, allowWithoutShopify });
  ...
  return { order: { ...order, shopify_total: shopifyOrder?.total_price ?? null }, lines };
}
 ...
 * re-delivered event for an already-delivered order is a no-op.
 *
 * `shipmentFees` / `codCollected` come from the Bosta payload and are stored
 * for payout reconciliation; they never affect revenue or COGS.
 */
async function recordDelivery(
  brandId,
  orderNumber,
  { codAmount = null, trackingNumber = null, deliveredAt = null, allowWithoutShopify = false, shipmentFees = null, codCollected = null } = {}
) {
  const resolved = await ensureOrderWithLines(brandId, orderNumber, { trackingNumber, allowWithoutShopify });
  ...
  if (Number.isFinite(Number(shipmentFees))) patch.shipment_fees = round2(Number(shipmentFees));
  if (Number.isFinite(Number(codCollected))) patch.cod_collected = round2(Number(codCollected));
