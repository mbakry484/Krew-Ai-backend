...
/** Small delay helper for transient-failure retries. */
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

/**
 * Rewrite a Shopify CDN image URL to request a bounded on-the-fly variant
 * instead of the full-resolution original. Shopify serves resized images via a
 * `width` query param. Smaller payloads eliminate >20MB / 413 / 400 size
 * failures at the vision API and shrink the surface that trips content
 * moderation. Non-Shopify or unparseable URLs pass through unchanged.
 * @param {string} url
 * @param {number} [width=1024]
 * @returns {string}
 */
function boundedImageUrl(url, width = 1024) {
  try {
    const u = new URL(url);
 ...
 * Validates that the response is actually an image so a stray HTML/error body
 * (e.g. a 200 error page) can't be base64'd and shipped to the vision model.
...
/**
 * Describe ONE image with GPT-4o vision using the shared structured schema.
 * Used by BOTH product indexing and customer-query search.
 *
 * Returns a result object instead of throwing on model-level failures, so the
 * caller can walk a fallback chain (alt image → text) rather than dropping the
 * product:
 *   { ok: true,  attributes, garmentType, summary, embeddingText, usage }
 *   { ok: false, reason: 'refused'|'empty', finish_reason, refusal }
 *
 * Retry policy: a single retry, only for genuinely transient outcomes — a
 * network/5xx error, or empty content WITHOUT a content_filter/refusal. A
 * content-filter/refusal is deterministic, so we never re-send the identical
 * payload; a 4xx (e.g. 400 image too large) is thrown for the caller to handle.
 *
 ...
 * @param {{label?: string}} [opts] label used only for log correlation
 */
async function describeImageForSearch({ base64, contentType }, catalogHints = {}, { label = 'image' } = {}) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    try {
      ...
      // Network / API-level error. Retry once only for transient failures
      // (no HTTP status, or a 5xx). A 4xx is deterministic — surface it so the
      // caller can move on to the next image instead of re-sending it.
      console.error(`❌ Vision API error [${label}] (attempt ${attempt}): ${err.message} | status=${err.status} | error=${JSON.stringify(err.error || null)}`);
    ...
    const choice = visionResponse.choices[0];
    const finish_reason = choice.finish_reason;
    const refusal = choice.message.refusal ?? null;
    const raw = choice.message.content || '';
    const usage = visionResponse.usage || null;

    // Deterministic content moderation / refusal — retrying the identical
    // payload cannot help, so report it and let the caller try another source.
    if (finish_reason === 'content_filter' || refusal) {
      console.warn(`⚠️  Vision refused [${label}]: finish_reason=${finish_reason} refusal=${JSON.stringify(refusal)} raw.length=${raw.length} raw="${raw.slice(0, 500)}"`);
      return { ok: false, reason: 'refused', finish_reason, refusal };
    }

    // Structured JSON — the happy path.
    try {
      ...
      if (parsed && typeof parsed === 'object' && parsed.summary) {
        return {
          ok: true,
          attributes: parsed,
          garmentType: normalizeGarmentType(parsed.type),
          summary: parsed.summary || buildEmbeddingText(parsed),
          embeddingText: buildEmbeddingText(parsed),
          usage
        };
      }
    } catch (_) { /* fall through to free-text / retry handling */ }

    // Non-empty but unparseable → usable free-text description (legacy behavior).
    if (raw.trim()) {
      console.warn(`⚠️  Vision returned non-JSON [${label}], using free text: ${raw.slice(0, 80)}...`);
      return { ok: true, attributes: null, garmentType: null, summary: raw, embeddingText: raw, usage };
    }

    // Empty content with no content_filter/refusal → transient; retry once.
    console.warn(`⚠️  Vision returned empty output [${label}] (attempt ${attempt}): finish_reason=${finish_reason} refusal=${JSON.stringify(refusal)} raw.length=${raw.length} raw="${raw.slice(0, 500)}"`);
    if (attempt < 2) await sleep(750);
  }

  return { ok: false, reason: 'empty', finish_reason: null, refusal: null };
}
...
/**
 * Build a text-only description for products whose images can't be described
 * (all vision attempts refused/failed). A text embedding from the title, type,
 * store description and image alt text is far more useful than no row at all —
 * without it a refused product is invisible to Luna's image search.
 * @param {object} product - { name, product_type?, description? }
 * @param {Array<{altText?: string}>} [images]
 * @returns {{attributes: null, garmentType: string|null, summary: string, embeddingText: string, usage: null}}
 */
function describeProductFromText(product, images = []) {
  const altText = images.map((i) => i && i.altText).filter(Boolean).join('. ');
  const text = [
    product.name ? `Type: ${product.name}.` : null,
    product.product_type ? `Category: ${product.product_type}.` : null,
    product.description ? `Description: ${product.description}` : null,
    altText ? `Image alt text: ${altText}.` : null,
  ].filter(Boolean).join(' ') || (product.name || 'product');

  const garmentType = normalizeGarmentType(product.product_type) || normalizeGarmentType(product.name);
  return { attributes: null, garmentType, summary: text, embeddingText: text, usage: null };
}
 ...
 * @param {object} product - { shopify_product_id, name, image_url, images?, product_type?, description? }
 *        `images` is the full [{ url, altText, ... }] array from the sync; when
 *        present we can try an alternate shot before falling back to text.
 * @returns {Promise<number[]|null>} The embedding vector or null if failed
 */
async function generateProductEmbedding(product) {
  // Prefer the full images array (so we can try an alternate shot when the
  // first is refused); fall back to the single image_url for older callers.
  const images = (Array.isArray(product.images) && product.images.length)
    ? product.images.filter((img) => img && img.url)
    : (product.image_url ? [{ url: product.image_url, altText: '' }] : []);
  ...
  const catalogHints = { productName: product.name, productDescription: product.description };

  try {
    let described = null;
    let source = null;

    // (a) Try up to 2 images: images[0], then one alternate. A flat-lay
    // alternate often passes moderation when the primary on-model shot is
    // refused, so a refusal on [0] shouldn't doom the whole product.
    const maxImageAttempts = Math.min(2, images.length);
    for (let i = 0; i < maxImageAttempts; i++) {
      const label = `${product.name} img[${i}]`;
      try {
        const url = boundedImageUrl(images[i].url, 1024);
        const image = await downloadImageAsBase64(url);
        const result = await describeImageForSearch(image, catalogHints, { label });
        if (result.ok) {
          described = result;
          source = i === 0 ? 'vision' : 'vision_alt';
          break;
        }
        console.warn(`⚠️  ${label}: vision unusable (reason=${result.reason}, finish_reason=${result.finish_reason ?? 'n/a'}) — trying next source`);
      } catch (err) {
        // Download failure or deterministic 4xx (e.g. image too large) — skip to next image.
        console.error(`❌ ${label}: image/vision error: ${err.message} | status=${err.status} | error=${JSON.stringify(err.error || null)}`);
      }
    }

    // (b) All image attempts failed/refused → embed from text rather than
    // dropping the product entirely (a refused product is otherwise invisible
    // to Luna's image search).
    if (!described) {
      described = describeProductFromText(product, images);
      source = 'text_fallback';
      console.warn(`📝 ${product.name}: all image attempts failed — using TEXT-only embedding`);
    }
    ...
    console.log(`📝 ${product.name} → source: ${source} | type: ${garmentType || 'unknown'} | ${described.embeddingText.substring(0, 100)}...`);
    ...
    console.log(`✅ Embedding generated for ${product.name} (source: ${source})`);
    return embedding;

  } catch (err) {
    console.error(`❌ Embedding failed for ${product.name}: ${err.message} | status=${err.status} | error=${JSON.stringify(err.error || null)}`);
    return null;
  }
}
...
async function generateEmbeddingsForBrand(brandId, { force = false, limit = null } = {}) {
  try {
    ...
    let query = supabase
      .from('products')
      .select('shopify_product_id, name, image_url, product_type, description, images')
      .eq('brand_id', brandId)
...
async function searchProductsByImage(brandId, queryImageUrl, limit = 5) {
  try {
    ...
    const described = await describeImageForSearch(image, {}, { label: 'query-image' });
    if (!described.ok) {
      console.error(`❌ Image search: vision unusable (reason=${described.reason})`);
      return [];
    }
