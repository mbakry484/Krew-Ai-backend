 ...
 * the same region of vector space. Do not fork this prompt per side — the
 * product side only ADDS a catalog-context block via buildVisionPrompt().
 */
...
async function generateEmbeddingsForBrand(brandId, { force = false, limit = null } = {}) {
  try {
    ...
    if (Number.isInteger(limit) && limit > 0) {
      ...
      console.log(`🔢 Embedding run capped at ${limit} products (testing mode)`);
    }
